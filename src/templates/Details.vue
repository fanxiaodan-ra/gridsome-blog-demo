...
<page-query>
    query($id: ID) {
        userFollowing: following(id: $id) {
            id
            login
        }
        userFollowers: followers(id: $id) {
            id
            login
        }
...
export default {
  ...
  computed: {
      githubUsername () {
        return this.$page.userFollowers?.login || this.$page.userFollowing?.login
      }
  },
  mounted() {
      this.getUser()
  ...
  methods: {
    async getUser() {
        try {
            this.loading = true
            const { data } = await axios.get(`https://api.github.com/users/${this.githubUsername}`)
            this.name = data.name
