        ...
        <div v-if="projects&&projects.length>0">
            <el-card shadow="hover" v-for="(item,index) in projects" :key="'pro'+index" style="margin-bottom: 20px" v-if="!item.hide">
                ...
                <div style="font-size: 0.9rem;line-height: 1.5;color: #606c71;">
                    最近更新 {{$util.utcToLocal(item.updated_at)}}
                </div>
            ...
            <div style="text-align: center">
                <el-pagination @current-change="list" background layout="prev, pager, next" :current-page.sync="currentPage" :page-size="5"
                    :total="5*totalPages">
                </el-pagination>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.totalPages = this.$page.allProjects.edges[0].node.totalPage
            this.projects = this.$page.allProjects.edges.map((i) => i.node)
        },
        methods: {
            async list(e) {
                this.loading = true
                this.currentPage = e
                const { data } = await axios.get(`https://api.github.com/users/${this.githubUsername}/repos?page=${e}&per_page=5`)
                this.projects = data.map(i => {
                  return {
                    id: i.id,
                    name: i.name,
                    url: i.html_url,
            ...
            goDetails(name) {
                this.$router.push("/pro-detail/" + name)
            },
            goGithub(url) {
                window.open(url)
            }
